...
def process_directory_content(base_path):
    """
    Génère le contenu des fichiers d'un répertoire.
    Yield: ('data', header, content, total_lines)
    """
    is_first_file = True
    total_lines = 0
...
def process_directory_architecture(base_path, folders_only=False):
    """
    Génère l'arborescence d'un répertoire.
    Yield: ('data', line, element_count)
    """
    element_count = 1
...
def process_project_directory(base_path):
    """
    Génère le contenu des fichiers d'un projet en excluant certains dossiers.
    Yield: ('data', header, content, total_lines)
    """
