            ...
            if c_type == "python_doc":
                ...
                if line_content.strip().startswith('//') or line_content.strip().startswith('/*'):
                    ...
                    if sl_index != -1:
                        ...
                        # Inline /* */ (plus rare sur une ligne avec du code, mais possible)
                        # Pour simplifier ici, si on détecte pas //, on vire la ligne si elle contient un bloc commentaire
                         new_lines = lines[:start_line] + lines[end_line + 1:]
